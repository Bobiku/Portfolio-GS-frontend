...
export class ProjetsService {
    ...
    private apiUrl = environment.apiUrl; // URL de base pour l'API Notion
    private notionVersion = environment.notionVersion; // Version de l'API
    private token = environment.token; // Remplace par ton token d'intégration
    private databaseId = environment.databaseId; // ID de la database

    constructor(private http: HttpClient) {}

    // Récupérer les pages de la base de données
    getDatabasesPages(databaseId: string): Observable<any> {
        const url = `${this.apiUrl}/databases/${databaseId}/query`;
        ...
        console.log('URL: ' + url);
        console.log('Body: ' + body.filter.property + ', ' + body.filter.multi_select.contains);
        console.log('Headers: ' + headers);
        const databasesPages = this.http.post(url, body, { headers });
        console.log(databasesPages);
        return databasesPages;
    }

    // Récupérer les blocs d'une page
    getPageBlocks(pageId: string): Observable<any> {
        const url = `${this.apiUrl}/blocks/${pageId}/children`;
        const headers = new HttpHeaders({
        'Authorization': this.token,
        'Notion-Version': this.notionVersion,
        });
